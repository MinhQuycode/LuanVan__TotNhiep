...
function TimeMovie() {
    ...
    let theaterTheoHeThong = stateShowtime?.filter((tile) => tile.theaters_id === codeTheater.maHeThongRap && tile.theater_branch_id === idTheaterChoose);
    // console.log(theaterTheoHeThong);
    ...
    const TimOfMovie = () =>{
      ...
      for (let index = 0; index < movieTheater?.length; index++) {
        ...
        // console.log(element);
        let time = theaterTheoHeThong.filter(item => item.movies_id === element);
        // console.log(time)
    ...
    const renderShowTimes = () =>{
      return (
        movieTheater?.map((showTime,index)=>{
          // console.log(showTime)
          return (
            <div key={index} className="chieu">
                    {
                      movie?.filter(item => item.id === showTime)
                      .map((img,index) =>{
                        return (
                          <div className="row" key={index}>
                            <div className="col-4">
                                <p className="name_movie text-center" style={{fontWeight:'bold',color:'orangered'}}>{img.name}</p>
                                <img src={img.image} alt="anh" />
                               <div className="mt-2 text-center" style={{fontWeight:'bold',fontSize:20}}>2D Digital</div>
                            </div>
                            <div className="col-8 info_lich">
                            <p style={{fontWeight:'bold',paddingLeft:'1rem'}}>Lịch chiếu :</p>
                            {
                              arrayTimeMovie?.map((item)=>{
                              // console.log(item)
                              return (
                              item.filter(date =>date.movies_id === img.id)
                              .map((times,index) =>{
                              // console.log(times)
                              return(
                                  <button key={index} className="btn__time m-2" onClick={()=>{
                                    handleChoiceShowsTime(times.id)
                                  }}> 
                                  <span style={{fontWeight:'bold', color:'green'}}>
                                    {times.time}~
                                  </span>
                                  {times.date.substring(8,10)}-{times.date.substring(5,7)}
                                  </button>
                              )})                    
                    )})
                  }
